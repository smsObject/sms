 ...
 * on 2014/12/3.
 */
public class CustomerParamMethodArgumentResolver implements HandlerMethodArgumentResolver {


    public CustomerParamMethodArgumentResolver() {
    }
    ...
    public final Object resolveArgument(MethodParameter parameter,
        ...
        String name = parameter.getParameterAnnotation(Param.class).value();
    ...
    protected Object createAttribute(String attributeName, MethodParameter parameter,
        ...
        if (ParamMap.class.isAssignableFrom(parameterType)) {
            return ParamMap.class.newInstance();
        }
    ...
    protected void bindRequestParameters(
        ...
        if (Collection.class.isAssignableFrom(targetType)) {//bind collection
        ...
        } else if (ParamMap.class.isAssignableFrom(targetType)) {
            ...
            ((ParamMap) binder.getTarget()).setMap(target);
            ...
            for (Object key : servletRequest.getParameterMap().keySet()) {
                ...
                if (isSimpleComponent(prefixName)) { //bind simple type
                    String[] paramValues = servletRequest.getParameterValues(prefixName);
                    Object val;
                    if (paramValues.length > 1) {  //多个值处理成逗号分隔的字符串
                        StringBuilder values = new StringBuilder();
                        for (Object value : paramValues) {
                            values.append(",").append(value.toString().trim());
                        }
                        val = values.deleteCharAt(0).toString();
                    } else {
                        val = paramValues.length == 0 ? "" : paramValues[0].trim();
                    }
                    target.put(keyValue, simpleBinder.convertIfNecessary(val, valueType));
                } else {
    ...
    private ServletRequest prepareServletRequest(Object target, NativeWebRequest request, MethodParameter parameter) {
        ...
        String modelPrefixName = parameter.getParameterAnnotation(Param.class).value();
    ...
    private String getNewParameterName(String parameterName, String modelPrefixName) {
        ...
        if (modelPrefixNameLength == 0) return parameterName;
    ...
    private boolean isFormModelAttribute(String parameterName, String modelPrefixName) {
        ...
        if (modelPrefixNameLength == 0) return true;
